...
MAX_EPISODES = 2000 #
MAX_EP_STEPS = 50  # iteration in each episodes (above)
DECADE_EPIS = 800
...
parameters[1] = [1.5, 5, 1, 1, 1]
parameters[2] = [8, -22, 12, 12, 30]
parameters[3] = [5, 0.01, 0.1]
    ...
    for j in range(MAX_EP_STEPS):
        ...
        if j == MAX_EP_STEPS - 1:
            ...
            for step in range(MAX_EP_STEPS):
                ...
                # a1, s1, parameter_a1, r1, s_1 = buff_pdqn.extract_data(step)
                # r1 += buff_pdqn.extract_punish(step) * 5
                #
                # a2, s2, parameter_a2, r2, s_2 = buff_dpdqn.extract_data(step)
                # r2 += buff_dpdqn.extract_punish(step) * 5

...
class PDQN(object):
    ...
    def store_transition(self, a, s, para_a, r, s_):
        ...
        if self.pointer[0] >= self.MEMORY_CAPACITY / 4:
            self.pointer[1] = 1
            # self.pointer[:, 1] = 1
    ...
    def _build_a(self, s, scope, trainable):
        ...
        with tf.variable_scope(scope):
            ...
            # net3 = tf.layers.dense(out2, 16, activation=tf.nn.leaky_relu, name='l3', trainable=trainable)
...
class Double_PDQN(object):
    ...
    def _build_a(self, s, scope, trainable):
        ...
        with tf.variable_scope(scope):
            ...
            # net3 = tf.layers.dense(out2, 64, activation=tf.nn.leaky_relu, name='l3', trainable=trainable)
            # out3 = tf.nn.dropout(net3, keep_prob=self.dropout_value)
...
class my_env(object):
    ...
    def get_states(self, a, parameter_a):
            ...
            for i in range(self.AP_num):
                ...
                for j in range(self.CU_num):
                    temp1 = sum(np.multiply(self.h[i][k, :],
                                            np.sqrt(0.8*AP_power) * ap_beam[i][j, :]))
                    temp2 = sum(np.multiply(np.conj(self.h[i][k, :]),
                                            np.sqrt(0.8*AP_power) * np.conj(ap_beam[i][j, :])))
                    total_interference[k] += np.real(np.multiply(temp1, temp2))
                if a[i] == 1:
                    temp1 = sum(np.multiply(self.h[i][k, :],
                                            np.sqrt(0.8*AP_power) * ap_beam[i][k, :]))
                    temp2 = sum(np.multiply(np.conj(self.h[i][k, :]),
                                            np.sqrt(0.8*AP_power) * np.conj(ap_beam[i][k, :])))
        ...
        s = 2 * np.append(throughput/10, normal_energy_harvest)
    ...
    def get_punish(self, a, parameter_a):
        ...
        for i in range(self.AP_num):
            ...
            for k in range(self.CU_num):
                AP_energy[i] += \
                    np.real(sum(np.real(np.multiply(np.sqrt(0.8*AP_power_constraint[i]) * ap_beam[i][k, :],
                                                    np.sqrt(0.8*AP_power_constraint[i]) * np.conj(ap_beam[i][k, :])))))
        ...
        punish1 = self.punish_factor[1] * (self.constraints[1] - 10 * s[self.CU_num:self.state_dim] / 2)
        punish2 = self.punish_factor[2] * (
        ...
        punish4 = self.punish_factor[4] * (self.constraints[4] - sum(AP_energy))
    ...
    def get_reward(self, a, parameter_a, pre_reward_cal):
        ...
        # reward = (- total_energy / 25 + punish * 10)
        # reward = ((pre_total_energy - total_energy) * 1 + sum(np.multiply(pre_cons - cons, self.punish_factor)) * 30) / 20
        # reward = ((pre_total_energy - total_energy) * 1 + punish * 30) / 10
        reward = punish * 1
        harvest = 10 * s[self.CU_num:self.state_dim] / 2
        reward_cal = [total_energy, a, parameter_a, cons,
                      update_class, update_beam, energy_info, AP_energy, throughput, harvest]
...
class temp_buff(object):
    ...
    def reward_modify(self, energy, throughput, harvest, ap):
        for i in range(self.len):
            if self.cons[1][i] - energy >= 0:
                self.buff[3][i] += - (self.cons[1][i] - energy)
            else:
                self.buff[3][i] += 0

            for j in range(len(self.cons[2][i])):
                if self.cons[2][i][j] - throughput[j] <= 0:
                    self.buff[3][i] += (self.cons[2][i][j] - throughput[j])
                else:
                    self.buff[3][i] += 0

            for j in range(len(self.cons[3][i])):
                if self.cons[3][i][j] - harvest[j] <= 0:
                    self.buff[3][i] += (self.cons[3][i][j] - harvest[j])
                else:
                    self.buff[3][i] += 0

            # for j in range(len(self.cons[4][i])):
            #     if self.cons[4][i][j] - ap[j] >= 0:
            #         self.buff[3][i] += - (self.cons[4][i][j] - ap[j]) / ap[j]
            #     else:
            #         self.buff[3][i] += 0

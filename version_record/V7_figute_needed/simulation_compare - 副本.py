...
parameters[0] = [1e-15, 0.95, 0.05, 20e6, 5e9]
parameters[1] = [0, 0, 1, 1, 10]
parameters[2] = [8, -23, 8, 8, 20]
parameters[3] = [5, 0.1, 0.1]
...
for i in range(MAX_EPISODES):
    ...
    for j in range(MAX_EP_STEPS):
        ...
        # [a, s, para_a, r, s_]
        data = [a1, s1, parameter_a1, 0, s_1]
        buff_pdqn.buff_update(data)
        # [pun, energy_consum, throughput, harvest, AP]
        ap_data = np.hstack((reward_cal1[7],np.sum(reward_cal1[7])))
        data = [pun1, reward_cal1[0], reward_cal1[8], reward_cal1[9], ap_data]
        buff_pdqn.cons_update(data)

        # [a, s, para_a, r, s_]
        data = [a2, s2, parameter_a2, 0, s_2]
        buff_dpdqn.buff_update(data)
        # [pun, energy_consum, throughput, harvest, AP]
        ap_data = np.hstack((reward_cal2[7], np.sum(reward_cal2[7])))
        data = [pun2, reward_cal2[0], reward_cal2[8], reward_cal2[9], ap_data]
        ...
        # throughput = parameters[0][3] * 1e-6 * np.log2(1 + np.power(10, s1[0:CU_num] / 10))
        throughput = 10 * s1[0:CU_num] / 2
        energy_harvest = 10 * s1[CU_num:CU_num + EU_num] / 2
        ...
        # throughput = parameters[0][3] * 1e-6 * np.log2(1 + np.power(10, s2[0:CU_num] / 10))
        throughput = 10 * s2[0:CU_num] / 2
        energy_harvest = 10 * s2[CU_num:CU_num + EU_num] / 2
        ...
        if j == MAX_EP_STEPS - 1:
            ...
            AP_con = [parameters[2][2], parameters[2][2], parameters[2][2], parameters[2][4]]

            if i >= 1:
                buff_pdqn.reward_modify(ep_reward[0][i-1][1], throughput_con, harvest_con, AP_con)
                buff_dpdqn.reward_modify(ep_reward[1][i-1][1], throughput_con, harvest_con, AP_con)
            else:
                buff_pdqn.reward_modify(80, throughput_con, harvest_con, AP_con)
                buff_dpdqn.reward_modify(80, throughput_con, harvest_con, AP_con)
            ...
            for step in range(MAX_EP_STEPS):
                ...
                pdqn.store_transition(a1, s1, parameter_a1, r1 / 50, s_1)
                double_pdqn.store_transition(a2, s2, parameter_a2, r2 / 50, s_2)

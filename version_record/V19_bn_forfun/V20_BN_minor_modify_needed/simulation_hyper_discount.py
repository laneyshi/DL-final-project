...
MAX_EPISODES = 800  #
MAX_EP_STEPS = 40  # iteration in each episodes (above)
DECADE_EPIS = 800
...
parameters[1] = [5, 5, 20]
parameters[2] = [5, -28, 5, 5, 13.5, 30]
parameters[3] = [2, 0.1, 0.05]

parameters[4] = [[3.5, 2.5], [-1, -4], [-2, 3]]
parameters[5] = [[2, 1.5], [0.5, 2.5], [-1, 0], [0.5, -1]]

mean_para = [[], []]
mean_para[0] = [-3, -7, -5, -7]
# violate factor
mean_para[1] = [0.03, 0.07, 0.05, 0.07]
# satisfy factor
...
discount = [0.90, 0.85, 0.80, 0.70, 0.10, 1 - 1e-3]
# # discount = [0.95, 0.05]
# discount = [0.95, 0.95]
double_pdqn = []
buff = []
...
memory_capacity = np.int(0.8*1e4)
batch_size = 16
clip_c = 3
clip_a = 3
...
for i in range(discount.__len__()):
    hyper_para = [lr_a, lr_c, discount[i], tau,
                  memory_capacity, batch_size, clip_c, clip_a, dropout_value_train, dropout_value_test]
    double_pdqn.append([])
...
ep_reward = np.zeros((discount.__len__(), MAX_EPISODES, 3))
...
PRE_PUN = np.zeros((discount.__len__(), MAX_EPISODES))
...
pre_reward_cal0 = [80, a, parameter_a0, 0]
...
for i in range(MAX_EPISODES):
    ...
    for q in range(discount.__len__()):
        buff[q].renew()
    ...
    for j in range(MAX_EP_STEPS):
        ...
        if j == MAX_EP_STEPS - 1:
            ...
            for q in range(discount.__len__()):
                ...
                PRE_PUN[q][i] = PRE_PUN[q][i] / MAX_EP_STEPS
                ep_reward[q][i][0] -= PRE_PUN[q][i]
...
# plot_DPDQN = my_plot_hyper.discount(parameters, ep_reward, discount, MAX_EPISODES)
# plot_DPDQN.punish()
...
DPDQN = save_hyper.discount(parameters, ep_reward, discount, MAX_EPISODES)
DPDQN.reward()
DPDQN.system_energy()

...
class Double_PDQN(object):
    ...
    def _build_a(self, s, scope, trainable):
        ...
        with tf.variable_scope(scope):
            net1 = tf.layers.dense(s, 256, activation=tf.nn.leaky_relu, name='l1', trainable=trainable)
            out1 = tf.nn.dropout(net1, keep_prob=self.dropout_value)

            net2 = tf.layers.dense(out1, 128, activation=tf.nn.leaky_relu, name='l2', trainable=trainable)
            out2 = tf.nn.dropout(net2, keep_prob=self.dropout_value)
            ...
            a = tf.layers.dense(out3, para_a_dim, activation=tf.nn.tanh, name='a', trainable=trainable)
...
class MY_PDQN(object):
    def __init__(self, state_dim, cla_dim, cu_num, eu_num, antenna_num, hyper_para, number):
        ...
        self.ae_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                           scope='PDQN' + str(self.number) + '/Actor/eval')
        self.at_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                           scope='PDQN' + str(self.number) + '/Actor/target')
        self.ce_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                           scope='PDQN' + str(self.number) + '/Critic/eval')
        self.ct_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                           scope='PDQN' + str(self.number) + '/Critic/target')
...
class my_env(object):
    ...
    def get_punish(self, a, parameter_a):
        ...
        punish0 = self.punish_factor[0] * -1 * (self.constraints[0] - throughput) / self.constraints[0]
        punish1 = self.punish_factor[1] * \
                  (self.constraints[1] - 100 * s[self.CU_num:self.state_dim]) / self.constraints[1]
        punish2 = self.punish_factor[2] * \
                  (AP_power_constraint[0:self.CU_num] - AP_energy[0:self.CU_num]) / AP_power_constraint[0:self.CU_num]
        punish3 = self.punish_factor[3] * \
                  (AP_power_constraint[self.CU_num:self.state_dim] - AP_energy[self.CU_num:self.state_dim]) \
                  / AP_power_constraint[self.CU_num:self.state_dim]
...
class temp_buff(object):
    def __init__(self, max_ep_step):
        ...
        # [a, s, para_a, energy_consum, s_, r , cons_num]

    def update(self, data):
        for i in range(self.buff.__len__()):
            self.buff[i].append(data[i])

    def average_modify(self, index, mean):
        for i in range(self.len):
            self.buff[index][i] = self.buff[3][i] - mean

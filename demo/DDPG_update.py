...
class PDQN(object):
    ...
    def store_transition(self, a, s, para_a, r, s_, e_ap_num):
        ...
        if self.pointer[e_ap_num, 0] == np.floor(MEMORY_CAPACITY_UNIT * self.k_list[e_ap_num].__len__() / 1):
            self.pointer[:, 1] = 1

    def _build_a(self, s, e_ap_num, scope, trainable):
        with tf.variable_scope(scope):

            net1 = tf.layers.dense(s, 64, activation=tf.nn.leaky_relu, name='l1', trainable=trainable)
            out1 = tf.nn.dropout(net1, keep_prob=self.dropout_value)

            net2 = tf.layers.dense(out1, 32, activation=tf.nn.leaky_relu, name='l2', trainable=trainable)
            ...
            a = tf.layers.dense(out3, this_a_dim, activation=tf.nn.tanh, name='a', trainable=trainable)
...
class my_env(object):
    ...
    def get_reward(self, a, parameter_a, pre_reward_cal):
        ...
        pre_total_energy, pre_a, pre_parameter_a, pre_punish = pre_reward_cal
        ...
        front_sinr = np.clip(np.power(2, (d_front_throughput + e_front_throughput) * 1e6 / self.sys_para[3]) - 1, 0, 50)
        ...
        punish = sum(self.get_punish(a, parameter_a))
        reward = ((pre_total_energy - total_energy) + punish * 20) / 10
        reward_cal = [total_energy, a, parameter_a, punish]
    ...
    def channel_change(self):
        for AP in range(self.AP_num):
            for user in range(self.state_dim):
                ...
                self.h[AP][user] += self.path_loss[AP][user] * this_delta
...
parameters[2] = [1, -100, 15, 15, 18 * (AP_num-1)]
parameters[3] = [18, 2, 0.8]
...
parameters[4] = [[5.6, 4.2], [0, -7], [-5.6, 4.2]]
parameters[5] = [[7, 1], [3, 8], [3, -6], [-7, -3]]
...
for i in range(MAX_EPISODES):
    ...
    for j in range(MAX_EP_STEPS):
        ...
        for E_AP_num in range(pdqn.beam_dim.__len__()):
            if pdqn.pointer[E_AP_num, 1] == 1:
                var[E_AP_num] *= .99995   # decay the action randomness, for a smaller var of gaussian value
                gate[E_AP_num] *= .99995
                learn_time = pdqn.k_list[E_AP_num].__len__()
                for k in range(learn_time):
                    pdqn.learn(E_AP_num)

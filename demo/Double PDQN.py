...
MAX_EPISODES = 300  #
MAX_EP_STEPS = 20  # iteration in each episodes (above)
    ...
    def learn(self):
        ...
        self.sess.run(self.ctrain,
                      {self.S: bs, self.K: bk, self.R: br[:, np.newaxis], self.S_: bs_,
                       self.K_: max_q_k, self.a: ba, self.dropout_value: DROPOUT_VALUE_TRAIN})
    ...
    def loss_check(self):
        ...
        critic_loss = \
            self.sess.run(self.critic_loss_check,
                      {self.S: bs, self.K: bk, self.R: br[:, np.newaxis], self.S_: bs_,
                       self.K_: max_q_k, self.a: ba, self.dropout_value: DROPOUT_VALUE_TEST})
...
double_pdqn = Double_PDQN(state_dim, cla_dim, CU_num, EU_num, Antenna_num)
...
for i in range(MAX_EPISODES):
    for j in range(MAX_EP_STEPS):
        ...
        double_pdqn.store_transition(a, s, parameter_a, r, s_)
